...
module Radarbox
  class AirportsApi
    ...
    def get_nat_tracks_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AirportsApi.get_nat_tracks ...'
      end
      # resource path
      local_var_path = '/airports/natTracks'
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AirportsApi#get_nat_tracks\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
